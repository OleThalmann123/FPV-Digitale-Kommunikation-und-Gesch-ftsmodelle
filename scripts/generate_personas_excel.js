...
// Avatar-Eigenschaften 1:1 aus src/app/page.tsx (roleEigenschaften). Erste
// Zeile ist jeweils die Jobbeschreibung, danach folgen Branchenwissen,
// Tools & Technologien, Soziale Kompetenz, Weitere Kenntnisse.
...
// Ausgeschriebene Jobbezeichnung pro Rolle. Die Jobbeschreibung steckt
// als erste Zeile in roleEigenschaften und kommt 1:1 aus page.tsx.
const roleJobbezeichnung = {
  'CEM (Customer Experience Manager)': 'Customer Experience Manager',
  'SMM (Social Media Manager)': 'Social Media Manager',
  'DMM (Digital Manager)': 'Digital Manager',
  'Growth Manager': 'Growth Manager',
  'Kommunikation Manager': 'Kommunikationsmanager',
  'Webseiten Manager': 'Website Manager',
};

const ROLES = Object.keys(roleJobbezeichnung);
const GESCHLECHTER = ['Männlich', 'Weiblich'];
const HAUSHALTE = ['1 Person kein Kind', '2 Personen 1 Kind'];

function buildAvatarText(role) {
  const eigLines = roleEigenschaften[role].split('\n');
  const jobbeschreibungLine = eigLines[0]; // "Jobbeschreibung: ..."
  const restEigenschaften = eigLines.slice(1).join('\n');
  return [
    `Jobbezeichnung: ${roleJobbezeichnung[role]}`,
    jobbeschreibungLine,
